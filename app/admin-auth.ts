...
export async function authenticateAdmin(email:string,password:string,host?:string):Promise<AdminSession|null>{
  ...
  if(!row||row.role!=="client_admin"||row.status!=="active"||row.projectStatus!=="active"||(row.adminHost&&host&&row.adminHost!==host)||!(await verifyPassword(password,row.passwordSalt,row.passwordHash)))return null;
