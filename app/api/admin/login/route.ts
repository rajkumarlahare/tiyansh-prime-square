...
export async function POST(request:Request){
  ...
  const body=await request.json().catch(()=>({})) as {email?:string,password?:string},email=String(body.email||"").trim().toLowerCase(),key=await keyFor(request,email),now=Date.now(),db=env.DB,host=(request.headers.get("host")||new URL(request.url).host).toLowerCase().split(":")[0];
  ...
  const session=await authenticateAdmin(email,String(body.password||""),host);
