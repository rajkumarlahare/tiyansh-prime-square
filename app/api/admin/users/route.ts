...
export async function DELETE(request:Request){
  ...
  await env.DB.batch([env.DB.prepare("DELETE FROM gallery WHERE project_id=?").bind(current.projectId),env.DB.prepare("DELETE FROM plots WHERE project_id=?").bind(current.projectId),env.DB.prepare("DELETE FROM settings WHERE project_id=?").bind(current.projectId),env.DB.prepare("DELETE FROM admin_users WHERE project_id=?").bind(current.projectId),env.DB.prepare("UPDATE projects SET status='deleted',public_host=NULL,admin_host=NULL,updated_at=? WHERE id=?").bind(new Date().toISOString(),current.projectId)]);
