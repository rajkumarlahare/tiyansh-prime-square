...
test("round header logo and mini logo badge are present", () => {
  ...
  assert.match(html, /\.brandmark\{overflow:hidden;border-radius:999px !important\}/);
...
test("Gallery and Location buttons are smaller", () => {
  assert.match(html, /\.pill\{width:118px;height:40px;padding:0 10px;border-radius:12px/);
  assert.match(html, /\.pill \.pin\{width:14px;height:17px/);
  assert.match(html, /\.pill \.gallery\{width:14px;height:14px/);
});

test("RPK share meta and bundled share image exist", async () => {
  assert.match(html, /og:title" content="Shree Banashankari Nagara"/);
  assert.match(html, /og:image" content="\/rpk-share-card\.png"/);
  await access(new URL("../public/rpk-share-card.png", import.meta.url));
});

test("WhatsApp and Call Now enhancer is present", () => {
  assert.match(html, /https:\/\/wa\.me\//);
  assert.match(html, /tel:\+/);
  assert.match(html, /data-rekixo-wa/);
  assert.match(html, /data-rekixo-call/);
  assert.match(html, /95386 02461/);
});
